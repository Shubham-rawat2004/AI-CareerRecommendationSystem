...
public class RecommendationController {
    ...
    @Operation(summary = "Generate recommendations", description = "Generate AI-powered career recommendations for a student")
    public ResponseEntity<ApiResponse<List<RecommendationDTO>>> generateRecommendations(@PathVariable Long userId) {
        try {
    ...
    @Operation(summary = "Get user recommendations", description = "Retrieve all recommendations for a specific user")
    ...
    @Operation(summary = "Update recommendation status", description = "Update status of a recommendation (ACCEPTED/REJECTED)")
    public ResponseEntity<ApiResponse<Void>> updateRecommendationStatus(
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("Recommendation status updated successfully", null));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.error(e.getMessage()));
        }
